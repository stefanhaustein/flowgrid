...
public class Model {
  ...
  public static void saveBytes(final Filesystem filesystem, final String filename, byte[] data) {
    try {
      ...
      if (!unchanged) {
        OutputStream os = filesystem.save(filename);
        os.write(data);
        os.close();
      }
    } catch (IOException e) {
  ...
  public static HutnWriter saveJson(final Filesystem filesystem, final String filename) {
    ByteArrayOutputStream baos = new ByteArrayOutputStream() {
      boolean saved;
      public void close() throws IOException {
        super.close();
        if (!saved) {
          saveBytes(filesystem, filename, toByteArray());
        }
      }
    };
    try {
