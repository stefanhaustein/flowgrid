...
public abstract class Container extends Artifact implements Iterable<Artifact> {
  ...
  public void ensureLoaded() {
    ...
    try {
      ...
      HutnObject json = Model.loadJson(model().platform.storageFileSystem(), jsonFilename());
    ...
    if (!jsonFilename().endsWith(".fgi")) {
      try {
        if (model().platform.metadataFileSystem().stat(jsonFilename() + SIGNATURE_CACHE_FILE_EXTENSION) != null) {
          signaturesJson = Model.loadJson(model().platform.metadataFileSystem(), jsonFilename() +
  ...
  private boolean syncChild(String fileName, HashSet<String> deleted,
    ...
    Filesystem storeageFs = model().platform.storageFileSystem();
    ...
    if (fileName.endsWith("/")) {
      ...
      if (storeageFs.stat(fullFileName + Module.MODULE_FILENAME) != null) {
        fullFileName += Module.MODULE_FILENAME;
        kind = "module";
      } else if (storeageFs.stat(fullFileName + "class.fgc") != null) {
    ...
    Stat stat = storeageFs.stat(fullFileName);
    ...
    if (!kind.equals("resource")) {
      todo.put(artifact, Model.loadJson(storeageFs, fullFileName));
    }
    return true;
  }
  ...
  public boolean sync() {
    ...
    String[] fileNameArray = EMPTY_STRING_ARRAY;
    try {
      fileNameArray = model().platform.storageFileSystem().list(qualifiedName());
    } catch (IOException e) {
      model().platform.log("Module directory does not exist: " + qualifiedName(), e);
    }
  ...
  public void saveSignatureCache() {
    ...
    HutnWriter writer = Model.saveJson(model().platform.metadataFileSystem(), jsonFilename() +
        SIGNATURE_CACHE_FILE_EXTENSION);
    writer.startObject();
